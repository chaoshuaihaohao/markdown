...
//	msg.msg_control = buf;
	if (sock)
		//tun_sendmsg
		sock->ops->sendmsg(sock, &msg, sizeof(*buf));
	else
...
void vhost_test_poll_init(struct vhost_test_poll *poll,
			  __poll_t mask /*, struct vhost_test_dev *dev */ )
{
...
#else
	vhost_test_poll_init(&poll, EPOLLIN);
#endif
